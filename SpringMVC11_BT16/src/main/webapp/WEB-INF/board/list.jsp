	    								...
	    								<tr>
	    									...
	    									<td class="table-cnt">
											    <c:choose>
											        ...
											        <c:when test="${vo.role == 'PROFESSOR'}">강사</c:when>									       
  			...
  			if(oper == "register"){ //등록
				...
				var attached_data = regForm.find("#attached_data").val();
  				...
  				if(attached_data){
  					...
  					$("#delete_attached_data_btn button").attr("class", "btn btn-sm btn-dark").text(" 첨부파일 삭제 (" + attached_data + ")" );				
  				} 			 					
...
//댓글----------------------------------------------------------------------------------------- 
	...
	function cmtView(data){	
		...
		$.each(data, function(index, obj){ //index:순서 표시자
			...
			if(obj.cmt_level == 0 && obj.cmt_available != '1'){
				listHtml += "<button type='button' class='btn btn-custom btn-sm'>대댓</button>";	
			}else if(obj.cmt_level == 0 && obj.cmt_available == '1'){
				listHtml += "<button type='button' class='btn btn-custom btn-sm'"; 		
				listHtml += "onclick='cmtComment(" + obj.cmt_idx + ")'>대댓</button>";
			}
			listHtml += "</td>";
					...
					<td colspan="6">
						...
						<form class="cmtcmtForm"> 
							...
							<table id="cmtcmtTbl" class="table table-bordered table-hover">
								<tr>				
									<td>
										<textarea id="cmtcmt_content" name="cmt_content" class="form-control" rows="2" cols="" placeholder="대댓글을 입력해주세요."></textarea>
									</td> 
									<td style="text-align:center; vertical-align:middle; width:70px;"> 
										<button class="btn btn-custom btn-sm" type="button" onclick="cmtCmtInsert(this)">대댓등록</button>											
									</td>
								</tr>
							</table>	
						</form>	
	...
	function cmtCmtInsert(btn){	
	    ...
	    if (contentField.val().trim() == "") {
	        ...
	        contentField.focus(); // 친절하게 커서도 깜빡여주자
	        return false;
	    }
		...
		$.ajax({
			...
			success : function(){ 
				...
				loadCmtcmt(); //비동기방식으로 댓글리스트 가져오기 기능	
	...
	function cmtComment(cmt_idx){
		...
		getCmt_group(cmt_idx);
		
		$("#cmt_" + cmt_idx).toggle(); //id="cmt_\${obj.cmt_idx}" //토글실행
	};
	...
	function cmtCmtView(data, cmt_idx){   
		...
		$.each(data, function(index, obj){ //index:순서 표시자
			...
			if (obj.cmt_available != 1) { 	//1이아니면 삭제된 표시상태								
				...
				listHtml += "onclick='cmtCmtDelete(" + obj.cmt_group + "," + obj.cmt_idx + "," + cmt_idx + ")'>대댓삭제</button>";		
			} 
			listHtml += "</td>";
