			...
			<div class="card-body">
				...
				<sec:authentication property="principal.member" var="mvo" />
					...
					<sec:authorize access="hasRole('ADMIN')">
						...
						<p style="text-align: center">내 교육과정 [ 
							<c:choose>
						        <c:when test="${mvo.cource == 'BACK'}">백엔드</c:when>
						        <c:when test="${mvo.cource == 'FRONT'}">프론트엔드</c:when>									       
						        <c:when test="${mvo.cource == 'DESIGN'}">UX/UI 디자인</c:when>									       
						        <c:otherwise>데이터분석</c:otherwise>
						    </c:choose>				
						]</p>
