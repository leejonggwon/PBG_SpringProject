...
public class MySocketHandler_Group extends TextWebSocketHandler {
		...
		if(msg.indexOf("#$nickName_")!= -1) { //처음 채팅방 입장
			for (WebSocketSession s : list) {
				...
				if(myGrSession.equals(otherGrSession)) {
					s.sendMessage(new TextMessage("<font style='color:pink; font-size:10px;'>"+myMemSession+"님이 입장하셨습니다</font>"));
				}	
			}
		}else { //내용전달
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		for (WebSocketSession s : list) {	
			...
			String otherGrSession = (String)sessionMap.get("gr_id");
			if(myGrSession.equals(otherGrSession)) {
				s.sendMessage(new TextMessage("<font style='color:blue; font-size:10px;'>"+myMemSession+"님이 퇴실하셨습니다("+now+")</font>"));
			}
		}
	}
